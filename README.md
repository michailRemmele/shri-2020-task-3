...
Пожалуй, можно и скрипты докинуть, чтобы превью стало интерактивным. Главное не забыть их подключить в index.html.
...
Кажется, что позиция ошибки в редакторе показывается не совсем верно. При этом значения передаются корректные. Возможно, я не правильно передаю эту информацию после линтинга в редактор.
Похоже дело в том, что отсчет значений в объекте Position, который нужно передать в редактор, начинается с нуля, а на вход приходит значение, где отсчет идет с единицы. Похоже достаточно будет просто отнять единицу от каждого значения. Для удобства и краткости вынесем это в отдельную функцию toPosition().

Да, теперь все работает как надо.

Теперь, пожалуй, немного наведем порядок в описании кодов с ошибками. Текстовые коды такие как WARNING.TEXT_SIZES_SHOULD_BE_EQUAL преобразуются во вложенную структуру, где WARNING это имя группы, а TEXT_SIZES_SHOULD_BE_EQUAL значение внутри нее. Из-за этого приходится делать split по точке кода, который приходит в GetSeverity() и затем уже вытаскивать значение severity. Нехорошо как-то. Преобразуем их лучше в более простые строчки. Для этого заведем функцию GetRuleKey(), которая конвертирует проблемное значение в более простое.

Оу. Чуть не пропустил. Как оказалось в свиче для получения уровня ошибки, есть ошибка: коду Severity.Error соответствует DiagnosticSeverity.Information. Нехорошо. Заметил когда начал переключать эти параметры в настройках VSCode.

Финальным штрихом можно снести **json-to-ast** зависимость, потому что она больше не нужна. На самом деле эта зависимость используется и в моем линтере, но я в проект добавил уже собранный бандл линтера, который содержит **json-to-ast**, поэтому из package.json ее можно удалить.
